...
static void unregisterLuaReference(LuaReference *r, lua_State *l) {
  if ((r != NULL) && (r->state != NULL) && (r->state == l) &&
      (r->ref != LUA_NOREF)) {
    luaL_unref(r->state, LUA_REGISTRYINDEX, r->ref);
    r->state = NULL;
    r->ref = LUA_NOREF;
...
static LuaWebView *lua_webview_newuserdata(lua_State *l) {
  ...
  lua_Integer height = luaL_optinteger(l, 4, 600);
  lua_Integer resizable = lua_toboolean(l, 5);
  int debug = lua_toboolean(l, 6);
  LuaWebView *lwv = (LuaWebView *)lua_newuserdata(
      l, sizeof(LuaWebView) + titleLen + 1 + urlLen + 1);
  ...
  lwv->webview.width = (int)width;
  lwv->webview.height = (int)height;
  lwv->webview.resizable = (int)resizable;
...
static const char *const lua_webview_loop_modes[] = {"default", "once",
  "nowait", NULL};
